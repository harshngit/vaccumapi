...
--   {BASE_URL}/uploads/{stored_filename}
...
CREATE TABLE IF NOT EXISTS uploads (
  ...
  stored_name     VARCHAR(255)  NOT NULL,      -- Saved filename: timestamp_originalname
  file_url        TEXT          NOT NULL,      -- Full public URL
  mime_type       VARCHAR(100)  NOT NULL DEFAULT 'image/jpeg',
  file_size_bytes INTEGER,
  entity_type     VARCHAR(50),                 -- 'job' | 'report' — which module owns it
  entity_id       VARCHAR(50),                 -- JOB-0001, RPT-0001, etc.
  uploaded_by_user_id INTEGER   REFERENCES users(id) ON DELETE SET NULL,
  uploaded_at     TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);

COMMENT ON TABLE  uploads            IS 'Tracks all locally uploaded files. URL is BASE_URL/uploads/stored_name';
...
COMMENT ON COLUMN uploads.entity_id   IS 'The ID of the owning entity e.g. JOB-0001, RPT-0001';

CREATE INDEX IF NOT EXISTS idx_uploads_entity ON uploads (entity_type, entity_id);
CREATE INDEX IF NOT EXISTS idx_uploads_uploaded_by ON uploads (uploaded_by_user_id);
